...
class BasicsCase(TestCase):
    ...
    def test_05_get_404(self):
        response = self.client.get(f"/dash/mamadousako/basics/")
        self.assertTrue(response.status_code, 404)

    def test_05_get_success(self):
        response = self.client.get(f"/dash/{self.emblem}/basics/")
        self.assertTrue(response.status_code, 200)

    def test_06_put_404(self):
        payload = {}
        response = self.client.put(
            f"/dash/fasjdnfapsijn/basics/",
